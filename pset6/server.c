...
{
    // a global variable defined in errno.h t
    hat's "set by system 
 ...
 */
char* indexes(const char* path)
{ int slasher = 0; //We need this int slasher to determine if we need to add a slash after the directory name.
    if (path[strlen(path)-1]!='/') {slasher = 1;}
    //Create a pointer large enough for our new path
    char* index_php_path= malloc((strlen(path)+1+slasher)*sizeof(char) + strlen("index.php")); 
    if(slasher == 1) {
    sprintf(index_php_path, "%s%c%s",path, '/' ,"index.php");}//Sprintf everything together with an extra leading '/'
    else {sprintf(index_php_path, "%s%s",path,"index.php");}
    index_php_path[strlen(index_php_path)] = '\0'; //Null terminate our new file.
    if(fopen(index_php_path, "r")) //If it opens, it is real. If not, move on. 
    {return index_php_path;}
    free(index_php_path); //Cleanup
    char* index_html_path= malloc((strlen(path)+1+slasher)*sizeof(char) + strlen("index.html")); //Same as above.
    if(slasher == 1)
    {sprintf(index_html_path, "%s%c%s",path, '/' ,"index.html");}
    else {sprintf(index_html_path, "%s%s",path ,"index.html");}
    if(fopen(index_html_path, "r"))
...
{   BYTE* data = malloc(sizeof(BYTE));
    BYTE* temp = malloc(sizeof(BYTE)); //We actually need two buffers for this. We read into temp byte by byte then save our file in data.
    *content = NULL;
    *length = 0;
    int file_length = 0; //Default values
    while (fread(temp, sizeof(BYTE), 1, file)>0) //Again, reading into temp Byte by Byte until 0 bytes are read by fread.
{
    
    data = realloc(data,file_length*sizeof(BYTE)+sizeof('\0')); //Keep increasing the size of data to accomodate our file.
    data[file_length] = *temp;//Write temp to data.
    file_length++; //Increase the integer we use to score filesize.
}
    data[file_length] = '\0'; //Make sure our file is null terminated.
        *content = data; //Write Data to content.
        
    *length = file_length; //Write file_length to length
    free(temp); //cleanup
    if (*content != NULL)
    {return true;} //Make sure nothing went wrong.
    return false;
}
 ...
 */
const char* lookup(const char* path)
{   //Pretty switch statement depending on value of path.
        if(strstr(path, ".css") != NULL) {return "text/css";}
        else if(strcasestr(path, ".html") != NULL) {return "text/html";}
        else if(strcasestr(path, ".gif") != NULL) {return "image/gif";}
        else if(strcasestr(path, ".ico") != NULL) {return "image/x-icon";}
        else if(strcasestr(path, ".jpg") != NULL) {return "image/jpeg";}
        else if(strcasestr(path, ".js") != NULL) {return "text/javascript";}
        else if(strcasestr(path, ".php") != NULL) {return "text/x-php";}
        else if(strcasestr(path, ".png") != NULL) {return "image/png";}
        else {return NULL;}
...
{
    //temp_line must be large enoguh to contain chars the size of line plus the null terminator \0
  char* temp_line = malloc((strlen(line)+1)*sizeof(char));
 //We have to copy line into temp_line to discard the constant char designation
 strcpy(temp_line, line);
 //We then make temp_line2 to make changes to, so we can free temp_line at the end of the program
 //to make Valgrind happy.
 char* temp_line2 = temp_line;
  
  char space[2] = " ";
  char* method_pointer = strsep(&temp_line2, space); //This takes the first part of temp_line2
  //in order to get the method
  if(strcasecmp(method_pointer, "GET") != 0) //One of our Parse requirements.
        {error(405); return false;}
    
  char* path_holder = strsep(&temp_line2, space); //This gets abs_path and query as a chunk.
  
char* http_pointer = strsep(&temp_line2,space); //This gets the Http Version as a chunk.
http_pointer[strlen(http_pointer)-2]='\0'; //Null terminating http_pointer, just to be safe.
if(strchr(path_holder, '\"')) //Another Parse requirement.
        {error(400); return false;}
if(strchr(path_holder,'?')) //Creates logic to see how we handle query.
{strcpy(abs_path,strsep(&path_holder,"?")); //Takes the abs_path half.
strcpy(query,strsep(&path_holder,"?"));} //Takes the query half.
else
{strcpy(abs_path,path_holder); //Because here path_holder=abs_path (no query)
strcpy(query,"");} //Query is specifically empty.
        if(strcasecmp(http_pointer, "HTTP/1.1")!=0) //Another Parse requirment
        {error(505); return false;} 
  
    if(abs_path[0]!= '/') //Another
        {error(501); return false;}

abs_path[strlen(abs_path)] = '\0'; 
if (strlen(query)>0)
{query[strlen(query)] = '\0';} //Null terminate both of these to be careful
