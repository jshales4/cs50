...
bool check_each(const char* word_to_find, node* checking_location)
{if(strcmp(word_to_find, checking_location->word) == 0)
    {return true;} 
else if (checking_location->next != NULL) 
    {check_each(word_to_find,checking_location->next);}
return false;}
...
{   char* word_local = malloc(sizeof(word));
    strcpy(word_local,word);
    for (int i = 0; strlen(&word[i]); i++)
    {word_local[i] = tolower(word[i]);}
    unsigned long hash_picker = hash1(word_local);
    if (hashtable[hash_picker]==NULL)
    {return false;}
    bool p = check_each(word_local, hashtable[hash_picker]);
    free(word_local);
    return p;
}
*/
